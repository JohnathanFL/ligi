...
pub const Token = struct {
    tag: Tag, pos: FilePos, str: []const u8
};
...
pub const Lexer = struct {
    ...
    fn matchWord(self: *Lexer) []const u8 {
        var len: usize = 1;
        while (self.hasLen(len + 1) and isWordChar(self.input[len])) {
            len += 1;
        }
...
test "recognize reserved" {
    try doTest(&[_]Tag{
        ...
        .In,       .NotIn,     .OpenRange, .ClosedRange, .Add,
        .Sub,      .Mul,       .Div,       .Mod,         .BitOr,
        .BitAnd,   .BitXor,    .Struct,    .Ref,         .Slice,
        .Array,    .Const,     .Comptime,  .BitNot,      .Not,
        .Opt,      .Pure,      .Inline,    .Overload,    .Property,
        .Access,   .Pipe,      .Using,     .Let,         .Var,
        .CVar,     .Field,     .Enum,      .Alias,       .If,

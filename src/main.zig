...
pub export fn strID(cache: *StringCache, str: [*:0]const u8, len: usize) callconv(.C) StrID {
    // printf("Interning {}!\n", .{str});
    return cache.strID(str[0..len]);
}
pub export fn idStr(cache: *StringCache, id: StrID) callconv(.C) [*:0]const u8 {
    return @ptrCast([*:0]const u8, cache.idStr(id));
}
